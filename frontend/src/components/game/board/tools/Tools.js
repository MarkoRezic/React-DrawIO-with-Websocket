            ...
            {context?.gameState?.drawing_user_id === context?.user?.user_id
                ? <>
                    <div id="color-picker-container">
                        Boja Olovke: &nbsp;
                        <input className={`${lightOrDark(context?.color)} `} type="color" value={context.color} onChange={(e) => { context.setColor(e.target.value) }} />
                    </div>
                    <div id="brushsize-container">
                        Veličina Olovke: &nbsp;
                        <input type='range' value={context.size} onChange={(e) => { context.setSize(e.target.value) }} min={1} max={100} />
                    </div>
                </>
                : null
            }
            <div id={`${context?.gameState?.drawing_user_id !== context?.user?.user_id ? 'guess' : 'draw'}-word-container`}>
                <span>{context?.gameState?.drawing_user_id === context?.user?.user_id ? 'Nacrtaj' : 'Pogodi'} riječ: &nbsp;</span>
                {
